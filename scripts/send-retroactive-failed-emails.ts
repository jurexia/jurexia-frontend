...
async function run() {
    ...
    console.log(`Se encontraron ${invoices.data.length} facturas abiertas.`);

    for (const invoice of invoices.data) {
        const email = invoice.customer_email;
        const attemptCount = invoice.attempt_count ?? 1;

        // Obtener email si invoice.customer_email esta vacío
        ...
        if (!targetEmail) {
            console.log(`⚠️ Sin email para la factura ${invoice.id}`);
            continue;
        }
        ...
        try {
            ...
            if (error) {
                console.error(`❌ Error enviando a ${targetEmail}:`, error);
            } else {
                console.log(`✅ Email enviado exitosamente a ${targetEmail}`);
            }
        } catch (e) {
            console.error(`❌ Error procesando para ${targetEmail}:`, e);
        }
    }
}
