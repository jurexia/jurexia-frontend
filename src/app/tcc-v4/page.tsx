...
/**
 * Redactor TCC v4 — Pipeline con humano en el loop.
 *
 * Flujo:
 *   1) Formulario (texto del acto + texto de conceptos/agravios)
 *   2) POST /redactor/tcc-v4/analyze (SSE) → recibe plan + job_id
 *   3) UI del secretario: lista lateral de problemas + detalle a la derecha.
 *      Atajos: J/K navegar, 1/2/3 marcar tesis, Ctrl+Enter siguiente.
 *   4) POST /redactor/tcc-v4/finalize (SSE) con job_id + edits → streamea estudio
 *
...
import { ArrowLeft, Loader2, Check, X, Star, AlertTriangle, FileText, Copy } from 'lucide-react';

const API_URL = process.env.NEXT_PUBLIC_API_URL || 'https://jurexia-api.onrender.com';

const TIPOS_ASUNTO = [
    { id: 'amparo_directo', label: 'Amparo Directo' },
    { id: 'amparo_revision', label: 'Amparo en Revisión' },
    { id: 'revision_fiscal', label: 'Revisión Fiscal' },
    { id: 'recurso_queja', label: 'Recurso de Queja' },
];
...
interface ProblemaPlan {
    ...
    razon_accion?: string;
    marco_normativo_a_transcribir?: any[];
    tesis_clave_a_citar?: TesisItem[];
    precedentes_clave_a_citar?: any[];
    subsuncion_concreta?: any;
    huecos_detectados?: string[];
    conclusion_razonada?: string;
}

interface PlanData {
    plan_por_problema?: ProblemaPlan[];
    validacion_global?: any;
}

interface EditState {
    calificacion: string;
    accion: string;
    tesisDecisions: Record<string, TesisDecision>; // registro → decision
    instruccion: string;
...
export default function TccV4Page() {
    ...
    // ─── Auto-save edits in localStorage ───
    useEffect(() => {
        if (!jobId) return;
        try {
    ...
    // ─── Keyboard shortcuts (review phase) ───
    ...
    const setTesisDecision = useCallback(
        (registro: string, decision: TesisDecision) => {
            ...
            setEdits((prev) => ({
                ...
                [activeProb.problema_id]: {
                    ...
                    tesisDecisions: {
                        ...prev[activeProb.problema_id].tesisDecisions,
                        [registro]: decision,
                    },
                },
            }));
    ...
    const startAnalyze = async () => {
        if (!textoActo.trim() || !textoConceptos.trim()) {
            setError('Pega el texto del acto reclamado y de los conceptos/agravios.');
            return;
        }
        setError('');
        setPhase('analyzing');
        ...
        try {
            ...
            while (true) {
                ...
                for (const ev of events) {
                    ...
                    if (!evData) continue;
                    let data: any;
                    try { data = JSON.parse(evData); } catch { continue; }

                    if (evType === 'phase') {
                        setProgress(data.detail || '');
                    } else if (evType === 'pass_complete') {
                        setProgress(`Pass ${data.pass} listo (${data.elapsed_s}s).`);
                    } else if (evType === 'plan_ready') {
                        setJobId(data.job_id);
                        setPlan(data.plan);
                        setPrecedentes(data.precedentes_utiles || []);
                        setPhase('review');
    ...
    const startFinalize = async () => {
        ...
        const editsPayload = {
            problemas: (plan.plan_por_problema || []).map((prob) => {
                ...
                const accionFinal = e.estado === 'inoperante_manual'
                    ? 'omitir_por_innecesario'
                    : e.accion;
                return {
        ...
        return (
            <div className="min-h-screen bg-cream-100 flex items-center justify-center p-4">
                <div className="bg-white border border-amber-300 rounded-lg p-8 max-w-md text-center">
                    ...
                    <p className="text-gray-600 mb-4">Redactor TCC v4 está en pruebas internas (solo admin).</p>
    ...
    const allResolved = problemas.every(
        ...
        <div className="min-h-screen bg-gray-50">
            <header className="bg-white border-b sticky top-0 z-10">
                <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-4">
                    ...
                    <h1 className="text-lg font-semibold">Redactor TCC v4 <span className="text-xs font-normal text-gray-500">— humano en el loop</span></h1>
                    {phase === 'review' && (
                        <span className="ml-auto text-sm text-gray-500">
                            atajos: <kbd className="px-1 bg-gray-100 border rounded">J</kbd>/<kbd className="px-1 bg-gray-100 border rounded">K</kbd> navegar · <kbd className="px-1 bg-gray-100 border rounded">Ctrl+↵</kbd> siguiente · (1/2/3 tesis llega en iteración 2)
                        </span>
                    )}
                </div>
            </header>
            ...
            {phase === 'form' && (
                <div className="max-w-3xl mx-auto p-6 space-y-4">
                    ...
                    <div>
                        <label className="text-sm font-medium block mb-1">Texto del acto reclamado / sentencia recurrida</label>
                        <textarea
                            className="w-full border rounded px-3 py-2 font-mono text-xs"
                            rows={10}
                            value={textoActo}
                            onChange={(e) => setTextoActo(e.target.value)}
                            placeholder="Pega aquí el texto completo del acto reclamado o la sentencia recurrida..."
                        />
                    </div>
                    <div>
                        <label className="text-sm font-medium block mb-1">Texto de los conceptos de violación / agravios</label>
                        <textarea
                            className="w-full border rounded px-3 py-2 font-mono text-xs"
                            rows={10}
                            value={textoConceptos}
                            onChange={(e) => setTextoConceptos(e.target.value)}
                            placeholder="Pega aquí los conceptos de violación o agravios..."
                        />
                    </div>
                    {error && (
                    ...
                    <button
                        ...
                        disabled={!textoActo.trim() || !textoConceptos.trim()}
                        className="bg-blue-600 hover:bg-blue-700 disabled:bg-gray-300 text-white px-6 py-2 rounded font-medium"
                    >
                        Analizar
                    </button>
                </div>
                    ...
                    <main className="col-span-9 bg-white rounded border p-5 h-[calc(100vh-100px)] overflow-y-auto">
                        ...
                        <div className="grid grid-cols-2 gap-3 mb-4">
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={(e) => setEdits((prev) => ({
                                        ...prev,
                                        [activeProb.problema_id]: { ...prev[activeProb.problema_id], calificacion: e.target.value },
                                    }))}
                                >
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={(e) => setEdits((prev) => ({
                                        ...prev,
                                        [activeProb.problema_id]: { ...prev[activeProb.problema_id], accion: e.target.value },
                                    }))}
                                >
                                        ...
                                        >
                                            <div className="flex items-start justify-between gap-3">
                                                ...
                                                <div className="flex gap-1 flex-shrink-0">
                                                    <button onClick={() => setTesisDecision(reg, 'rector')} title="Rector (1)" className={`p-1.5 rounded ${dec === 'rector' ? 'bg-green-600 text-white' : 'bg-gray-100 hover:bg-gray-200'}`}><Star className="w-4 h-4" /></button>
                                                    <button onClick={() => setTesisDecision(reg, 'apoyo')} title="Apoyo (2)" className={`p-1.5 rounded ${dec === 'apoyo' ? 'bg-blue-600 text-white' : 'bg-gray-100 hover:bg-gray-200'}`}><Check className="w-4 h-4" /></button>
                                                    <button onClick={() => setTesisDecision(reg, 'descartar')} title="Descartar (3)" className={`p-1.5 rounded ${dec === 'descartar' ? 'bg-gray-600 text-white' : 'bg-gray-100 hover:bg-gray-200'}`}><X className="w-4 h-4" /></button>
                                                </div>
                                            </div>
                                        </div>
                                    );
                        ...
                        <div className="mb-4">
                            ...
                            <textarea
                                ...
                                onChange={(e) => setEdits((prev) => ({
                                    ...prev,
                                    [activeProb.problema_id]: { ...prev[activeProb.problema_id], instruccion: e.target.value },
                                }))}
                            ...
                            {(['resuelto', 'diferido', 'inoperante_manual'] as const).map((s) => (
                                <button
                                    ...
                                    onClick={() => setEdits((prev) => ({
                                        ...prev,
                                        [activeProb.problema_id]: { ...prev[activeProb.problema_id], estado: s },
                                    }))}
            ...
            {(phase === 'finalizing' || phase === 'done') && (
                <div className="max-w-4xl mx-auto p-6">
                    ...
                    <div
                        ref={resultRef}
                        className="bg-white border rounded p-6 prose prose-sm max-w-none whitespace-pre-wrap font-serif"
                    >
...
// Keyboard 1/2/3 shortcut for tesis decisions — needs activeProb in scope, so define inside component scope above.
// (Implemented inline via the same useEffect that handles J/K to keep one listener.)
